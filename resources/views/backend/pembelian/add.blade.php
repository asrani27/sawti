          ...
          <div class="col-4">
            <div class="card card-outline card-primary">
                ...
                <form class="form-horizontal" method="POST" action="{{route('tambahkeranjang')}}">
                  ...
                  <div class="card-body">
                    <div class="form-group row">
                      <label for="inputEmail3" class="col-sm-3 col-form-label">Bibit</label>
                      <div class="col-sm-9">
                          <select name="bibit_id" class="form-control form-control-sm" required>
                              <option value="">-pilih-</option>
                ...
                <table class="table table-bordered table-striped table-sm">
                  ...
                  <tbody>
                      @foreach ($keranjang as $item)
                        <tr>
                            <td>{{$no++}}</td>
                            <td>{{$item->bibit->kode}}</td>
                            <td>{{$item->bibit->jenis}}</td>
                            <td>{{$item->jumlah}}</td>
                            <td>{{$item->bibit->stok + $item->jumlah}}</td>
                            <td>@currency($item->bibit->harga_beli)</td>
                            <td>@currency($item->jumlah * $item->bibit->harga_beli)</td>
                            <td>
